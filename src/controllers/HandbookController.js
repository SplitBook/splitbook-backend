...
module.exports = {
    ...
    async store(req, res) {
        ...
        try {
            ...
            return res.status(409).json({ error: "There is already a manual with that ISBN" });
        }
    ...
    async edit(req, res) {
        ...
        if (handbook) {
            try {
                const [,[updatedHandbook]] = await Handbook.update({ isbn, name }, {returning:true, where: { id } });
